...
impl StatsPane {
    ...
    fn ingest(&mut self, v: Value) {
        ...
        let mem_used_b = if inactive_file > 0.0 {
            (usage - inactive_file).max(0.0)
        } else if cache > 0.0 {
            (usage - cache).max(0.0)
        } else {
            usage
        ...
        let mut cpu_pct = if pre_total > 0 && pre_system > 0 && system_total > 0 {
            ...
            if sys_delta > 0.0 {
                (cpu_delta / sys_delta) * (self.last_ncpu as f64) * 100.0
            } else {
                0.0
            }
        ...
        if let Some(prev) = self.last {
            ...
            if cpu_pct <= 0.0 && cpu_total == prev.cpu_total_raw {
                cpu_pct = prev.cpu_pct;
            }
        }
    ...
    fn draw_cpu_card(&self, f: &mut Frame, area: Rect, theme: Theme) {
        ...
        let title = Line::from(vec![
            ...
            Span::styled(
                format!("(total {})  ", fmt_pct(total_pct)),
        ...
        let scale_lbl = if max_cpu < 1.0 {
            format!("(scale 0–{:.2}%)", max_cpu)
        } else {
            format!("(scale 0–{:.0}%)", max_cpu)
        };
...
fn pad(r: Rect, pad: u16) -> Rect {
    Rect {
        x: r.x + pad,
        y: r.y + pad,
        width: r.width.saturating_sub(2 * pad),
        height: r.height.saturating_sub(2 * pad),
    }
}
...
fn make_bar_lines(cols: &[f64], h: usize, from: Color, to: Color) -> Vec<Line<'static>> {
    ...
    for row in (0..h).rev() {
        ...
        for &v in cols {
            let filled_rows = if v <= 0.0 {
                0
            } else {
                (v * h as f64).ceil() as usize
            };
