...
#[inline] pub fn ease_out_cubic(t: f32) -> f32 { let u = 1.0 - t; 1.0 - u*u*u }
...
pub fn rocket_up_from_bar(p: f32) -> f32 {
    ...
    if p < 0.15 { 0.05 * ease_out_cubic(p / 0.15) }
    else if p < 0.60 { 0.05 + 0.60 * ease_out_cubic((p - 0.15) / 0.45) }
    else if p < 0.98 { 0.65 + 0.25 * ease_in_out_cubic((p - 0.60) / 0.38) }
    else { 0.90 + 0.10 * ((p - 0.98) / 0.02).clamp(0.0, 1.0) }
}

pub fn rocket_down_from_bar(p: f32) -> f32 {
    let p = p.clamp(0.0, 1.0);
    if p < 0.20 { 0.05 * ease_out_cubic(p / 0.20) }
    else if p < 0.70 { 0.05 + 0.60 * ease_in_out_cubic((p - 0.20) / 0.50) }
    else if p < 0.98 { 0.65 + 0.30 * ease_in_out_cubic((p - 0.70) / 0.28) }
    else { 0.95 + 0.05 * ((p - 0.98) / 0.02).clamp(0.0, 1.0) }
}

/// Rendu ASCII de la scène verticale (fusée + fond)
pub fn render_rocket_scene_vertical(kind: ActionKind, w: u16, h: u16, progress: f32, tick: u64) -> String {
    let w = w as usize;
    let h = h as usize;
    ...
    for x in 0..w { buf[ground_y][x] = if x % 2 == 0 { '‾' } else { ' ' }; }
    for x in 0..w { if x % 4 == 0 { buf[ground_y.saturating_sub(1)][x] = '.'; } }
    ...
    let layers = [(22usize, 10u64, '·', 0.35f32), (34, 6, '˙', 0.65f32), (52, 4, '·', 1.00f32)];
    for (den, speed, ch, par) in layers {
        let mut rng = StdRng::seed_from_u64(0x51DE + speed);
        let count = (w * h / den).max(10);
        let dy = ((parallax_shift as f32 * par).round() as isize + (tick as isize / (speed as isize * 5)))
            .clamp(-10000, 10000);
        for _ in 0..count {
    ...
    let y_world: isize = match kind {
        ActionKind::Starting => (start_on_ground as isize) + ((end_off_top - start_on_ground as isize) as f32 * pe) as isize,
        ActionKind::Stopping => (end_off_top as isize) + ((start_on_ground as isize - end_off_top as isize) as f32 * pe) as isize,
    };

    // Wobble
    let wobble = {
        let amp = if matches!(kind, ActionKind::Starting) { 1.0 - progress } else { progress };
    ...
    for (i, line) in ROCKET.iter().enumerate() {
        ...
        if y < 0 || (y as usize) >= h { continue; }
        let yy = y as usize;
        for (j, ch) in line.chars().enumerate() {
            let xx = x_left + j;
            if xx < w && yy < ground_y { buf[yy][xx] = ch; }
        }
    }
    ...
    for dy in 0..(flame_h as isize) {
        let y = base_y + dy;
        if y < 0 || (y as usize) >= ground_y { continue; }
        for dx in [-2isize, 0, 2] {
            ...
            let ch = match ((dy as usize) + flicker) % 4 { 0 => '╽', 1 => '│', 2 => '╿', _ => '│' };
        ...
        if near_ground == 1 {
            ...
            for _ in 0..puffs {
                ...
                if y >= 1 && (y as usize) < ground_y {
                    buf[y as usize][xx] = if (tick + xx as u64) % 2 == 0 { '·' } else { ',' };
                }
            }
        }
    }
    ...
    if matches!(kind, ActionKind::Stopping) && base_y + 1 >= ground_y as isize {
        ...
        for x in start..end {
            if (x + tick as usize) % 2 == 0 {
                ...
                buf[y][x] = if (x + (tick as usize / 2)) % 3 == 0 { '~' } else { '=' };
            }
        }
    }
