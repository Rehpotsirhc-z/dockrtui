...
impl ContainersView {
    ...
    async fn remove_ids(&mut self, items: Vec<(String, String)>) -> Result<()> {
        ...
        if ok > 0 { self.note_ok(format!("🗑 removed: {ok}")); }
        if !ko.is_empty() { self.note_err(format!("❌ failed: {}", ko.join(", "))); }
        Ok(())
    }
    ...
    pub async fn on_key(&mut self, key: KeyEvent) -> Result<()> {
            ...
            match p {
                ...
                Popup::ConfirmDelete { .. } => match key.code {
                    ...
                    KeyCode::Enter | KeyCode::Char('y') => {
                        let items = if let Some(Popup::ConfirmDelete { items }) = self.popup.take() { items } else { vec![] };
        ...
        if self.searching {
            match key.code {
                KeyCode::Esc => { self.searching = false; self.query.clear(); }
            ...
            if cur >= vis_len { self.state.select(if vis_len==0 { None } else { Some(vis_len-1) }); }
            return Ok(());
        }
        ...
        match key.code {
            ...
            KeyCode::Char('p') => {
                if let Some((id, name, state)) = self.current() {
                    match state.as_deref() {
                        Some("running") => {
                            match self.docker.pause(id).await {
                                Ok(_) => { self.note_ok(format!("⏸ paused {name}")); let _ = self.refresh().await; }
                                Err(e) => self.note_err(format!("❌ pause: {e}")),
                            }
                        }
                        Some("paused") => {
                            match self.docker.unpause(id).await {
                                Ok(_) => { self.note_ok(format!("▶ unpaused {name}")); let _ = self.refresh().await; }
            ...
            KeyCode::Char('P') => {
                let items: Vec<(String, String, Option<String>)> = self
                    ...
                    .filter_map(|c| {
                        ...
                        if !self.selected_ids.contains(&id) { return None; }
                        let name = c.names.as_ref().and_then(|v| v.get(0)).cloned().unwrap_or_else(|| id.clone());
                ...
                if items.is_empty() {
                    ...
                    for (id, name, st) in items {
                        match st.as_deref() {
                            Some("running") => {
                                if let Err(e) = self.docker.pause(&id).await { errs.push(format!("{name}: {e}")); }
                                else { paused += 1; }
                            }
                            Some("paused") => {
                                if let Err(e) = self.docker.unpause(&id).await { errs.push(format!("{name}: {e}")); }
                                else { unpaused += 1; }
                            }
                            _ => {}
                        }
                    ...
                    let _ = self.refresh().await;
                    if paused > 0 { self.note_ok(format!("⏸ paused: {paused}")); }
                    if unpaused > 0 { self.note_ok(format!("▶ unpaused: {unpaused}")); }
                    if !errs.is_empty() { self.note_err(format!("❌ {}", errs.join(", "))); }
                }
            }
            ...
            KeyCode::Char('O') => { self.sort_asc = !self.sort_asc; }
            ...
            KeyCode::Char('C') => { self.selected_ids.clear(); }
            KeyCode::Char('A') => {
                ...
                let ids: Vec<(String,String,bool)> = self
                    ...
                    .filter_map(|c| {
                        ...
                        if !self.selected_ids.contains(&id) { return None; }
                        let name = c.names.as_ref().and_then(|v| v.get(0)).cloned().unwrap_or_else(|| id.clone());
                        let running = matches!(c.state, Some(bollard::models::ContainerSummaryStateEnum::RUNNING));
                ...
                if ids.is_empty() {
                    ...
                    for (id, name, running) in ids {
                        let kind = if running { ActionKind::Stopping } else { ActionKind::Starting };
            ...
            KeyCode::Enter | KeyCode::Char(' ') => {
                if let Some((id, name, state)) = self.current() {
                    let kind = if matches!(state.as_deref(), Some("running")) { ActionKind::Stopping } else { ActionKind::Starting };
            ...
            KeyCode::Char('R') => {
                if let Some((id, name, _)) = self.current() {
                    let id = id.to_string();
                    let name = name.clone();
                    self.queue.push_back((ActionKind::Stopping, id.clone(), name.clone()));
            ...
            // Export
            KeyCode::Char('S') => {
                match self.save_visible_to_tmp() {
            ...
            KeyCode::Delete | KeyCode::Char('d') => {
                if let Some((id, name, _)) = self.current() {
                    self.popup = Some(Popup::ConfirmDelete { items: vec![(id.to_string(), name)] });
                } else {
                    self.note_warn("⚠ no container selected");
                }
            }
            ...
            // Remove (batch)
            KeyCode::Char('D') => {
                let items: Vec<(String, String)> = self.visible_indices()
                    .into_iter()
                    .filter_map(|i| self.rows.get(i))
                    .filter_map(|c| {
                        let id = c.id.as_deref()?.to_string();
                    ...
                    .collect();
                if items.is_empty() { self.note_warn("⚠ no selection"); }
                else { self.popup = Some(Popup::ConfirmDelete { items }); }
            }
    ...
    pub fn on_tick(&mut self) {
        ...
        if self.anim.is_none() {
            if let Some((kind, id, name)) = self.queue.pop_front() {
                if let Ok(anim) = futures_lite::future::block_on(
                    actions::launch_action(self.docker.clone(), kind, id, name)
                ) {
                    self.anim = Some(anim);
                }
            }
        ...
        if let Some(mut anim) = self.anim.take() {
            ...
            while let Ok(upd) = anim.rx.try_recv() {
                ...
                if matches!(anim.bar_phase, BarPhase::Done | BarPhase::Error) && anim.done_at.is_none() {
            ...
            let step = speed * dt;
            if diff.abs() <= step { anim.bar_pct = anim.bar_target_pct; }
            else { anim.bar_pct += step * diff.signum(); }
            ...
            let docker_done = anim.done_flag.load(std::sync::atomic::Ordering::Relaxed) && anim.bar_pct >= 0.999;
            let rocket_done = anim.rocket_t >= 0.999;

            if docker_done && rocket_done {
                if let Some(res) = anim.result.lock().unwrap().take() {
                    match (anim.kind, res) {
                        (ActionKind::Starting, Ok(_)) => self.note_ok(format!("▶ started {}", anim.name)),
                        (ActionKind::Stopping, Ok(_)) => self.note_ok(format!("⏹ stopped {}", anim.name)),
    ...
    fn move_sel(&mut self, delta: i32) {
        let vis_len = self.visible_indices().len();
        if vis_len == 0 { self.state.select(None); return; }
    ...
    fn current(&self) -> Option<(&str, String, Option<String>)> {
        ...
        let name = row.names.as_ref().and_then(|v| v.get(0)).cloned().unwrap_or_else(|| id.to_string());
        let state = row.state.as_ref().map(|s| s.to_string());
        Some((id, name, state))
    }

    fn note_ok<S: Into<String>>(&mut self, msg: S)   { self.last_note = Some((msg.into(), self.theme.ok)); }
    fn note_warn<S: Into<String>>(&mut self, msg: S) { self.last_note = Some((msg.into(), self.theme.warn)); }
    fn note_err<S: Into<String>>(&mut self, msg: S)  { self.last_note = Some((msg.into(), self.theme.err)); }
    ...
    pub fn draw(&mut self, f: &mut Frame, area: Rect) {
        ...
        if !self.query.is_empty() { hint.push(Span::styled(format!(" | filter: '{}'", self.query), Style::default().fg(theme.accent))); }
        hint.push(Span::styled(format!(" | sort: {sort_name}{arrow}"), Style::default().fg(theme.muted)));
        if sel_count > 0 { hint.push(Span::styled(format!(" | selected: {sel_count}"), Style::default().fg(theme.accent))); }
        if just_refreshed { hint.push(Span::styled(format!(" {spin}"), Style::default().fg(theme.accent))); }
        let bar = Paragraph::new(Line::from(hint)).block(theme.block("List"));
        f.render_widget(bar, top[0]);

        // Table — filtered + sorted view
        let vis = self.visible_indices();
        let header = Row::new(vec!["", "NAME", "IMAGE", "STATE", "PORTS"])
        ...
        let rows = vis.iter().enumerate().map(|(i, &idx)| {
            ...
            let icon = match c.state.as_ref() {
                Some(bollard::models::ContainerSummaryStateEnum::RUNNING) => Span::styled("●", Style::default().fg(lerp(theme.ok, theme.accent, pulse))),
                Some(bollard::models::ContainerSummaryStateEnum::PAUSED)  => Span::styled("■", Style::default().fg(theme.warn)),
                Some(bollard::models::ContainerSummaryStateEnum::EXITED)  => Span::styled("●", Style::default().fg(theme.err)),
                _                                                         => Span::raw("·"),
            };
            let id   = c.id.as_deref().unwrap_or_default();
            let name_raw = c.names.as_ref().and_then(|v| v.get(0)).cloned().unwrap_or_default();
            let name = if self.selected_ids.contains(id) { format!("▣ {}", name_raw) } else { format!("▢ {}", name_raw) };

            let image = c.image.clone().unwrap_or_default();
            let state_txt = c.state.as_ref().map(|s| s.to_string()).unwrap_or_else(|| "unknown".to_string());
            ...
            let ports = c.ports.as_ref().map(|ps| {
                ps.iter().map(|p| {
                    ...
                    if pubp > 0 { format!("{pubp}->{priv}/{typ}", priv=privp, typ=typ) }
                    else        { format!("{priv}/{typ}",     priv=privp, typ=typ) }
                }).collect::<Vec<_>>().join("  ")
            }).unwrap_or_default();
            ...
            if i == selected_row {
                let breath = ((self.tick % 40) as f32 / 40.0 * std::f32::consts::TAU).sin() * 0.5 + 0.5;
                let sel_bg = lerp(theme.accent_alt, theme.accent, breath);
                row = row.style(Style::default().bg(sel_bg).fg(Color::Black).add_modifier(Modifier::BOLD));
            } else if i % 2 == 1 {
        ...
        if let Some(anim) = &self.anim {
            ...
            let inner = Layout::default()
                .direction(Direction::Vertical)
                .constraints([Constraint::Length(2), Constraint::Min(h.saturating_sub(7)), Constraint::Length(5)])
                .split(Rect {
                    x: overlay.x + 2,
                    y: overlay.y + 1,
            ...
            let title = Paragraph::new(Line::from(vec![
                ...
                Span::styled(&anim.name, Style::default().fg(self.theme.fg).add_modifier(Modifier::BOLD)),
            ]));
            f.render_widget(title, inner[0]);

            let rocket = render_rocket_scene_vertical(anim.kind, inner[1].width, inner[1].height, anim.rocket_t, self.tick);
            ...
            let bar_line = fancy_bar_line(inner[2].width as usize, anim.bar_pct, self.tick, self.theme.accent, self.theme.accent_alt);
            let bar_para = Paragraph::new(bar_line)
                .block(Block::default().borders(Borders::ALL).title(format!("Progress ({phase_label})")));
        ...
        if let Some(Popup::Inspect(txt)) = &self.popup {
            ...
            f.render_widget(Clear, overlay);
            let inner = Rect { x: overlay.x + 1, y: overlay.y + 1, width: overlay.width - 2, height: overlay.height - 2 };
            let block = Block::default()
                .borders(Borders::ALL)
    ...
    fn save_visible_to_tmp(&self) -> Result<std::path::PathBuf> {
        ...
        for idx in vis {
            ...
            let name = c.names.as_ref().and_then(|v| v.get(0)).cloned().unwrap_or_default();
            let image = c.image.clone().unwrap_or_default();
...
fn match_visible(c: &ContainerSummary, tokens: &[String], label_filters: &[(String, Option<String>)]) -> bool {
    // free-text search
    let name = c.names.as_ref().and_then(|v| v.get(0)).cloned().unwrap_or_default().to_lowercase();
    let image = c.image.clone().unwrap_or_default().to_lowercase();
    let state = c.state.as_ref().map(|s| format!("{:?}", s)).unwrap_or_default().to_lowercase();
    let ports = c.ports.as_ref().map(|ps| {
        ps.iter().map(|p| {
            let privp = p.private_port;
            let pubp  = p.public_port.unwrap_or(0);
    ...
    let text_ok = tokens.iter().all(|t|
        name.contains(t) || image.contains(t) || state.contains(t) || ports.contains(t)
    );

    if !text_ok { return false; }

    // label filters (@key or @key=value)
    if label_filters.is_empty() { return true; }
    ...
    label_filters.iter().all(|(k, v)| {
        ...
        for (lk, lv) in labels {
            if lk.to_lowercase().contains(k) {
                if let Some(vv) = v {
                    if lv.to_lowercase().contains(vv) { found = true; break; }
...
fn format_inspect(name: &str, ins: &bollard::models::ContainerInspectResponse) -> String {
    ...
    let status  = state.and_then(|s| s.status.as_ref()).map(|x| format!("{:?}", x)).unwrap_or_else(|| "unknown".to_string());
    let health  = state.and_then(|s| s.health.as_ref()).and_then(|h| h.status.as_ref()).map(|x| format!("{:?}", x)).unwrap_or_else(|| "-".to_string());
    let started = state.and_then(|s| s.started_at.as_ref()).map(|x| x.as_str()).unwrap_or("-");
    let finished= state.and_then(|s| s.finished_at.as_ref()).map(|x| x.as_str()).unwrap_or("-");

    s.push_str(&format!("# {name}\n\n"));
    s.push_str(&format!("ID      : {id}\n"));
    s.push_str(&format!("Image   : {image}\n"));
    s.push_str(&format!("State   : {status}  (running={running}, health={health})\n"));
    s.push_str(&format!("Started : {started}\n"));
    if !running { s.push_str(&format!("Finished: {finished}\n")); }

    if let Some(cfg) = ins.config.as_ref() {
        if let Some(env) = cfg.env.as_ref() {
            if !env.is_empty() {
                s.push_str("\n## Env\n");
                for e in env.iter().take(40) { s.push_str(&format!("- {e}\n")); }
                if env.len() > 40 { s.push_str(&format!("… (+{} env)\n", env.len()-40)); }
            }
        }
    }

    if let Some(networks) = ins.network_settings.as_ref().and_then(|n| n.networks.as_ref()) {
        s.push_str("\n## Networks\n");
    ...
    if let Some(mounts) = ins.mounts.as_ref() {
        if !mounts.is_empty() {
            ...
            for m in mounts {
                ...
                let typ = m.typ.as_ref().map(|t| format!("{:?}", t)).unwrap_or_else(|| "-".to_string());
...
fn toggle(set: &mut HashSet<String>, id: &str) {
    if set.contains(id) { set.remove(id); } else { set.insert(id.to_string()); }
}
